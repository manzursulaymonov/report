    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          ...
          cornerRadius: 8,
          displayColors: false,
          callbacks: { label: c => 'MRR: $' + fmt(c.raw) }
        }
      },
      scales: {
...
if(eComp){
  ...
  new Chart(eComp,{
    ...
    data: {
      labels: dr.labels,
      datasets: [
        { label: 'Yangi', data: dr.addedMRR, backgroundColor: '#20c997', borderRadius: 3, borderSkipped: false },
        { label: 'Kengayish', data: dr.expMRR, backgroundColor: '#1746a2', borderRadius: 3, borderSkipped: false },
        { label: 'Qisqarish', data: dr.conMRR, backgroundColor: '#f97316', borderRadius: 3, borderSkipped: false },
        { label: 'Churn', data: dr.lostMRR, backgroundColor: '#c42b1c', borderRadius: 3, borderSkipped: false }
      ]
    },
    options: {
      ...bo,
      ...
      plugins: {
        legend: {
          ...
          labels: {
            ...
            font: { size: 11, family: "'Outfit',system-ui,sans-serif" },
        ...
        tooltip: {
            ...
            callbacks: {
                ...
                footer: items => {
                  const total = items.reduce((s,i)=>s+(i.dataset.label==='Yangi'||i.dataset.label==='Kengayish'?i.raw:-i.raw),0);
                  return 'Net: ' + (total>0?'+':'') + '$' + fmt(total);
                }
            }
        }
      },
